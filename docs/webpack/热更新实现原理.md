...
随后是应用`HotModuleReplacementPlugin`插件：
...
因此`module.hot`等`api`经过`parser`后会变为对应的`dependency`，在`code generate`时，调用对应的`template`生成新的代码：
...
在`seal`阶段，所有模块的代码生成之后，会调用`additionalTreeRuntimeRequirements`钩子，用于添加模块在代码生成时需要的`runtime`代码。如果使用了热更新，那么此时也会添加热更新相关的`runtime`代码：
...
首先会对`chunk`下的`module`的`hash`值进行对比。对比完成后，得到被更新了的`module`。随后为这些更新后的`module`创建一个新的`chunk`，为`HotUpdateChunk`：
