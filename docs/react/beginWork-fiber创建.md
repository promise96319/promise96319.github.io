...
function performUnitOfWork(unitOfWork: Fiber): void {
  ...
  // 并且将 fiber 的 flags 进行了标记，用于 替换，移除等等
...
总共分为两步，一是`beginWork`，二是`completeWork`。`beginWork`主要是向下”递“的过程，根据节点创建`fiber`。`completeWork`是向上”归“的过程，由下至上根据`fiber`创建或复用真实节点。
...
此时调用的`React.createPortal`方法创建的节点:
...
当`$$typeof`为`REACFT_ELEMENT_TYPE`的时候，会调用`reconcileSingleElement`方法。
...
在`reconcileSingleElement`方法里会校验`key`和`type`是否一致，如果一致则使用`useFiber`方法进行复用`fiber`。`useFiber`方法又会调用`createWorkInProgress`方法：
...
如果`current.alternate`不存在，那么会以`current`为基准，创建新的`fiber`，**并且新的fiber和current之间通过alternate属性建立联系。除此之外，新的fiber还会复用current的stateNode，通常来讲就是复用了fiber对应的真实节点。**
...
如果`shouldConstruct(type)`为`true`，说明`isReactComponent`存在。那么该属性在哪里定义的呢？我们在写类组件的时候通常会`extends Component`，在`ReactBaseClass.new.js`文件中找到`Component`的定义：
...
其他的都是`React`定义的一些`type`，比如`REACT_FRAGMENT_TYPE`、`REACT_SUSPENSE_TYPE`等，这里需要注意的是`type`为`object`的时候，`REACT_PROVIDER_TYPE`、`REACT_CONTEXT_TYPE`等`fiber`也是在这个位置创建的。
...
以一个简单的例子开始，加入节点有`A，B，C，D，E`5个老的`fiber`，现在新节点解析出来的是`A，B，D，E，F`5个`ReactElement`。最开始时我们需要依次从头到尾进行对比(主要是对比`key`和`type`两个属性)，发现`A，B`是可以复用的，但是`C`和`D`不一致，不能复用，此时退出当前对比部分。`React`这部分对比的代码如下：
