...
在上一章中，`beginWork`阶段对每一个不同的标签都创建了对应的`fiber`，接下来就是对`fiber`进行一些更加精细的处理。
...
第一步是判断是否有`context`，如果有的话`didRecieveUpddate`为`true`。然后获取`instance`:
...
`instance`为`class`组件的实例，存储在`fiber.stateNode`上。如果`instance`不存在，说明`class`组件还没有被实例化。

### constructClassInstance方法

首先第一步是判断是否有`contextType`属性。如果存在说明使用了`context`，此时会进行`readContext`，将`context`形成链表形式，存放到`fiber.dependences`上：
...
// 将 fiber 存储到实例上 _reactInternals
...
最后如果有`componentDidMount`，那么会将`fiber`标记为`Update`：
...
`updateClassInstance`与`resumeMountClassInstance`大同小异，区别是在当`componentDidUpdate`时将`fiber`标记为`Update`。
...
如果前后的`value`值未发生变化，那么跳过该节点的协调，否则调用`propagateContextChange`方法，该方法会以当前节点为根节点向下进行深度遍历。如果节点`fiber`上的`dependences`存在，说明使用了`context`。遍历`dependences`看是否有使用`Provider`变动对应的`context`。
...
如果查找到节点为`Provider`，那么就会跳过，不做任何处理。
...
此时遇到的是`Consumer`标签，首先会执行`prepareToReadContext`方法：
...
`readContext`的过程就是将`context`添加到`dependences`当中并形成链表结构：
...
在`beginWork`的时候先获取到`render`，然后其他操作同函数式组件，会进行`renderWithHookds`。
...
`updateFragment`也比较简单，相当于只包装了一层空壳，不对这层壳做处理，二是直接处理其子元素：
...
如果`current`存在，那么`current.child`就是被`memo`的组件的`fiber`。接着判断组件有没有被更新：
...
在创建`fiber`时，它的`stateNode`属性是一个包含了`containerInfo`的对象：
