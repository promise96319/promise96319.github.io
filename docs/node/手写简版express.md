...
这里实现的代码比较简单，没有涉及到`router`的处理，主要是概括了`express`中间件的实现原理。**核心原理大致是：将use和get/post/...等方法注册的回调函数封装成layer形式，并推到stack当中。等用户发送请求时，去队列中一次按照路径和方法名匹配对应的layer。然后按照“洋葱模型”执行匹配到的layer。**