...
| processFor  | v-for="(item, index) in arr" | el.for = "arr"el.alias = "item"el.iterator1 = "index"el.iterator1 = undefined (遍历对象的时候存在) |
...
| processSlotContent | v-slot:default="scope"                   | el.slotTarget=defaultel.slotTargetDynamic=动态绑定的名称el.slotScope=scopeel.children=只有在组件上使用才有children |
